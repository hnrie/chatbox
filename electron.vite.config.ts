...
/**
 * Vite plugin to set web-specific page title and meta description.
 */
export function injectWebMetadata(): Plugin {
  return {
    name: 'inject-web-metadata',
    transformIndexHtml(html) {
      return html
        .replace('<title>Chatbox</title>', '<title>Chatbox Web</title>')
        .replace('content="chatbox"', 'content="Chatbox — AI chat client for the browser"')
    },
  }
}
...
const rendererConfig = ({
  ...
  isWeb: boolean
  isDesktop: boolean
  inferredRelease: string
  inferredDist: string | undefined
}): UserConfig => ({
  resolve: {
    alias: {
  ...
  plugins: [
    ...
    isWeb ? injectWebMetadata() : undefined,
    visualizer({
  ...
  define: {
    ...
    'process.env.DEV_WEB_ONLY': JSON.stringify(process.env.DEV_WEB_ONLY || ''),
  },
  optimizeDeps: {
...
export default defineConfig(({ mode }) => {
  ...
  if (isWeb) {
    return {
      renderer: rendererConfig({ isProduction, isWeb, isDesktop, inferredRelease, inferredDist }),
